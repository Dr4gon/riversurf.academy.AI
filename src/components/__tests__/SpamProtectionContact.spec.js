...
async function fillAndSubmitForm(testName, wrapper, name, message, wait = 0) {
...
test('Submit contact form quickly', async () => {
  ...
  await fillAndSubmitForm('Submit contact form quickly', wrapper, 'John Doe', 'This is a test message');
  expect(wrapper.html()).toContain('submitMessageFalse');
});

test('Submit contact form after delay', async () => {
  const wrapper = mount(ContactComponent);
  await fillAndSubmitForm('Submit contact form after delay', wrapper, 'John Doe', 'This is a test message', 4100);
  expect(wrapper.html()).toContain('submitMessageRight');
});

test('Submit contact form with honeypot after delay', async () => {
  const wrapper = mount(ContactComponent);
  await fillAndSubmitForm('Submit contact form with honeypot after delay', wrapper, 'John Doe', 'This is a test message', 4100);
  expect(wrapper.html()).toContain('submitMessageFalse');
});

test('Submit contact form with honeypot quickly', async () => {
  const wrapper = mount(ContactComponent);
  await fillAndSubmitForm('Submit contact form with honeypot quickly', wrapper, 'John Doe', 'This is a test message');
  expect(wrapper.html()).toContain('submitMessageFalse');
});
